...
## What this is
Phone-first, on-device movement and form-degradation coach for iQOO Hackathon 2026 (Track: HealthTech). Final scope covers six standing exercises, released through exit-gated iterations (`iterations.md`).

## Product in one line
Live camera pose tracking runs one configurable evaluation pipeline across six exercises. It detects incomplete/incorrect visible range and persistent movement change, then reports one evidence-based rep status and local progress analytics without clinical claims.
...
- Office Kit (10%) — mirror live phone demo to laptop; no app subsystem (decision.md D6).
...
- Build in `iterations.md` order. Stop only at a passing iteration; hide exercises that have not passed their vertical-slice gate.
- Persist summary evidence only. Never retain camera frames, landmark streams, or raw pose samples.
- Keep Android screen awake while app is foregrounded so display timeout cannot interrupt exercise flow.
